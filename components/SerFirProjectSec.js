...
const SerFirProjectSec = (props) => {
    ...
    return (
        <section className="container mx-auto 2xl:mt-10 xl:mt-7 sm:mt-5 mt-4 py-3 md:px-5 sm:px-7 px-3">
            ...
            <button className={`left-0 absolute lg:text-2xl py-4 px-4 rounded-full shadow-xl bg-zinc-200 text-zinc-800 transition z-[5] xl:hidden duration-200 ease-in hover:bg-zinc-300`} onClick={SlideLeft}>
                <FaChevronLeft/>
            </button>
            <div className="flex 2xl:space-x-10 lg:space-x-7 space-x-5 overflow-x-scroll scroll scroll-smooth scrollbar-hide lg:mt-5 mt-4 xl:mx-0 md:mx-3 mx-2" ref={slider}>
                    {props.projectList.map((curVal) => (
                       <Link href={curVal.link} key={curVal.id} legacyBehavior>
                            <div className="bg-transparent border border-zinc-300 w-full rounded-xl cursor-pointer duration-200 ease-in hover:bg-[#ebfffb] xl:min-w-0 lg:min-w-[300px] min-w-[250px]" ref={scrollDiv}>
                                <div>
                                    <Image
                ...
                {/* ================ Right Button ================= */}
                <button className={`right-0 absolute lg:text-2xl py-4 px-4 rounded-full shadow-xl bg-zinc-200 text-zinc-800 transition z-[5] xl:hidden duration-200 ease-in hover:bg-zinc-300 `} onClick={SlideRight}>
                    <FaChevronRight/>
                </button>
            </div>
        </section>
...
export default SerFirProjectSec;